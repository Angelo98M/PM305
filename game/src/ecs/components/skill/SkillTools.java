...
public class SkillTools {
    ...
    public static void takeKnockback(Point source, Entity taker){
        PositionComponent pc = (PositionComponent)taker.getComponent(PositionComponent.class).get();
        Point takerPos = pc.getPosition();
        Point direction = Point.getUnitDirectionalVector(takerPos,source);
        taker.getComponent(VelocityComponent.class).ifPresent(
            vc -> {
                ((VelocityComponent)vc).setCurrentXVelocity(direction.x*0.2f);
                ((VelocityComponent)vc).setCurrentYVelocity(direction.y*0.2f);
            });
