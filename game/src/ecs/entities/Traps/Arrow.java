...
public class Arrow extends Traps {
    ...
    private Animation arrowAnimation = AnimationBuilder.buildAnimation("dungeon/Traps/runRight_arrow");
    public Arrow() {
        super();
    ...
    private void setArrowTrap(Entity Trap, Entity Projectile) {
        ...
        if (floor.contains( currentLevel.getTileAt(new Coordinate(wall.x - 1, wall.y)))) {
            ...
            destination =currentLevel.getTileAt(new Coordinate(wall.x-i+1,wall.y)).getCoordinateAsPoint();
            i = rnd.nextInt(4)+1;
            while (!floor.contains( currentLevel.getTileAt(new Coordinate(wall.x - i, wall.y)))) {
                i = rnd.nextInt(4)+1;
            }
            ...
            destination =currentLevel.getTileAt(new Coordinate(wall.x,wall.y-i+1)).getCoordinateAsPoint();
            i = rnd.nextInt(4)+1;
            while (!floor.contains( currentLevel.getTileAt(new Coordinate(wall.x, wall.y - i)))) {
                i = rnd.nextInt(4)+1;
            }
    ...
    public Point getPosition(){
        PositionComponent help = (PositionComponent) this.getComponent(PositionComponent.class).get();
